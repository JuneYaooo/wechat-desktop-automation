...
1. `click` 左侧“通讯录”图标，再点列表顶部“新朋友”（都在主窗口内，每步用 `capture` 核验）。
2. 待处理申请置顶显示，带“接受/添加到通讯录”按钮；“已添加/已过期”均为已完结记录，不得重复处理。
3. 没有待处理申请时如实报告并停止；不拿历史记录模拟测试。
4. `click` 指定申请的“接受”，弹窗保留默认备注与设置后 `click` “确定”（弹窗超出主窗口时用 `screen` 核验）。
5. 通过后若用户要求打招呼，按普通发送流程 draft → 预览 → send；找不到指定申请或多个待处理无法区分时，停下来询问用户。
